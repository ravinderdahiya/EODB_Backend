...
import { ensureRuntimeConfigEntries } from "./src/api-url/runtime-config.service.js";

async function upsertFrontendRuntimeUrls() {
  const result = await ensureRuntimeConfigEntries(prisma);
  console.log(`Runtime config URLs ensured in api_urls (created: ${result.created}/${result.total})`);
}
